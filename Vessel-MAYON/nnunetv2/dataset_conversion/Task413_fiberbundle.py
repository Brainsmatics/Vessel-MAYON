...
if __name__ == "__main__":
    ...
    train_dir = r'E:\huangjiang\project\nnUNet\dataset\nnUNet_raw\Task413_fiberbundle\imagesTr'
    test_dir = r'E:\huangjiang\project\nnUNet\dataset\nnUNet_raw\Task413_fiberbundle\imagesTs'
    output_folder = r'E:\huangjiang\project\nnUNet\dataset\nnUNet_raw\Task413_fiberbundle'

    json_dict = OrderedDict()
    json_dict['name'] = "Fiber Bundle"
    json_dict['description'] = "Fiber Bundle"
    ...
    file_name_test_ids = []
    for name in file_name_list:
        file_name_test_ids.append(name.split('.nii.gz')[0])

    json_dict['numTraining'] = len(file_id_list)
    json_dict['training'] = [{'image': "./imagesTr/%s.nii.gz" % i, "label": "./labelsTr/%s.nii.gz" % i} for i in
                             file_id_list]
    json_dict['numTest'] = 0
    json_dict['test'] = [{'test': "./imagesTs/%s.nii.gz" % i} for i in file_name_test_ids]
